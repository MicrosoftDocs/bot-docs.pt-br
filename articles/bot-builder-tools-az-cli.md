...
```
Os valores permitidos para `--kind` são: `function, registration, webapp` e para `--version` são `v3, v4`.  Se você não especificar o argumento `--lang`, um bot .NET é criado. Para criar um bot de nó, use `Node`.
