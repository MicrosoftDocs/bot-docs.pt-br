...
O Assistente Virtual baseia-se em recursos do [Modelo do Enterprise](./bot-builder-enterprise-template-overview.md) que fornecem recursos fundamentais para experiências de conversação, incluindo intenções de conversação básicas em vários idiomas, Expedição, QnA e insights de conversa. Atualmente, os recursos de assistente a seguir estão disponíveis. Outros recursos estão sendo planejados e estaremos trabalhando de perto com os clientes e parceiros para criar um roteiro.
...
Habilitação de habilidades | Atualmente há um grande conjunto de recursos comuns que exigem desenvolvimento por cada desenvolvedor. Nossa solução Assistente Virtual inclui um novo recurso de habilidades, que permite que novos recursos sejam conectados a um Assistente Virtual somente por meio de configuração e fornece um mecanismo de autenticação para que o recurso de habilidades solicite tokens para atividades downstream.
...
Integração do dispositivo | Nossos SDKs do Serviço de Bot do Azure (DirectLine), junto com o cartão adaptável e os SDKs de Fala facilitam a integração de plataforma cruzada com os dispositivos. Outros exemplos de integração de dispositivos e plataforma, incluindo o Edge, estão planejados.
...
Implantação automatizada | Todos os recursos do Azure de que o Assistente precisa são implantados automaticamente: registro do Bot, Serviço de Aplicativo do Azure, LUIS, QnAMaker, Content Moderator, CosmosDB, Armazenamento do Azure e Application Insights. Além disso, modelos do LUIS para todas as habilidades, do QnAMaker e de expedição são criados, treinados e publicados para habilitar o teste de imediato.
...
O Assistente Virtual abrange vários cenários do setor. Alguns cenários de exemplo são mostrados abaixo para fins de referência.
