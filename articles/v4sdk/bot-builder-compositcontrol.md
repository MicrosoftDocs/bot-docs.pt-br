---
title: Criar um conjunto integrado de diálogos | Microsoft Docs
...
ms.locfileid: "46707152"
---
# <a name="create-an-integrated-set-of-dialogs"></a>Criar um conjunto integrado de diálogos
...
O método usa o [contexto de turno](bot-builder-concept-activity-processing.md#turn-context) e um objeto de estado como parâmetros.
...
Para controlar a entrada do usuário, passaremos do diálogo pai um objeto `userInfo` como um parâmetro de diálogo. Em cada classe de diálogo, o diálogo será construído no construtor, que permite salvar informações em `userInfo`. Ao longo desse diálogo é possível gravar em um objeto de estado local definido como uma propriedade no objeto `step.values`, na medida em que o usuário insere informações. Após a conclusão do diálogo, o objeto de estado local será descartado. Portanto, salvaremos o objeto de estado local no pai `userInfo`, que persistirá informações sobre o usuário em todas as conversas que você tiver com o usuário. 
...
Para saber mais sobre diálogos e cascatas, veja [Usar diálogos para gerenciar o fluxo de conversas simples](bot-builder-dialog-manage-conversation-flow.md).
...
Uma vantagem de usar um diálogo componente é a capacidade para redigir diálogos compostos. Como cada `DialogSet` mantém um conjunto exclusivo de `dialogs`, compartilhamento ou referência cruzada `dialogs` não pode ser feito facilmente. É aqui que entra o diálogo componente. É possível usar diálogos componentes para criar um diálogo composto que facilita o gerenciamento do fluxo de diálogo entre diálogos. Para ilustrar isso, vamos criar mais dois diálogos: um para perguntar ao usuário qual mesa ele gostaria de reservar para o jantar e outro para criar um serviço de despertador. Novamente, usaremos uma classe separada para cada diálogo e cada diálogo será estendido `ComponentDialog`.
